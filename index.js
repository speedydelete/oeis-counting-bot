...
function wrapn(func) {
    return (...args) => func(...args.map(Number));
}
let scope = new Map();
...
scope.set('π', Math.PI);
scope.set('abs', (x) => typeof x === 'bigint' ? (x < 0 ? -x : x) : Math.abs(x));
scope.set('floor', (x) => typeof x === 'bigint' ? x : Math.floor(x));
scope.set('ceil', (x) => typeof x === 'bigint' ? x : Math.ceil(x));
...
scope.set('sign', (x) => typeof x === 'bigint' ? (x < 0) : Math.sign(x));
scope.set('trunc', (x, ndigits) => {
    if (typeof x === 'bigint') {
        return x;
    }
    else if (!ndigits) {
...
function _evaluate(node) {
    switch (node.type) {
        ...
        case 'UnaryExpression':
            let arg = _evaluate(node.argument);
        ...
        case 'BinaryExpression':
            ...
            let left = _evaluate(node.left);
            let right = _evaluate(node.right);
            switch (node.operator) {
                case '+':
                    return left + right;
                case '-':
        ...
        case 'OptionalCallExpression':
            ...
            let value = _evaluate(node.callee);
...
function evaluate(msg) {
    let value = _evaluate(parseExpression(msg));
    if (value === undefined || value === null) {
        return 0n;
    }
...
let config = JSON.parse((await fs.readFile('config.json')).toString());
let stats;
...
async function fail(message, msg, react = true) {
    if (previousSeq) {
...
client.on('messageCreate', async (message) => {
    ...
    if (message.content.startsWith('!')) {
        ...
        if (cmd === 'help') {
            ...
            let data = Object.entries(stats[cmd.slice(3)]);
